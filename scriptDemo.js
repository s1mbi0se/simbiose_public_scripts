...
/*
	...
	for(let i=0; i<5; i++) {
		statusOrder[i].children[0].childNodes[1].addEventListener('change', function(){
			let allPlayers = statusOrder[i].parentNode.parentNode.parentNode.parentNode.children[1].children[0].children
			//console.log(allPlayers[0].parentNode.innerHTML)
			let array = [...allPlayers]
			//let array = [].slice.call(allPlayers);
			let arraySize = array.length - 1
			//array.splice(arraySize, 0, allPlayers[0])
			array.shift()
			//allPlayers = [...array]
			console.log(array[0])
			console.log(statusOrder[i].parentNode.parentNode.parentNode.parentNode.children[1].children[0])
			statusOrder[i].parentNode.parentNode.parentNode.parentNode.children[1].children[0].innerHTML = ''
			for(let j=0; j<array.length; j++){
				statusOrder[i].parentNode.parentNode.parentNode.parentNode.children[1].children[0].innerHTML += `<div class="playersContainer">${array[j].innerHTML}</div>`
				//statusOrder[i].parentNode.parentNode.parentNode.parentNode.children[1].children[0].textContent += array[j]
				//console.log(allPlayers[j].children[1].children[0].children[1].textContent)
			}
		})
	}
*/
...
let statusOrder = document.getElementsByClassName('second-line')
	for(let i=0; i<5; i++) {
		statusOrder[i].children[0].childNodes[1].addEventListener('change', function() {
			...
			executeAction('query-select-to-order-asc', null, {identifier: 1}).then((result) => {
				teste = result
				for(let j=0; j<teste.length; j++){
					//console.log(teste[j])
					statusOrder[i].parentNode.parentNode.parentNode.parentNode.children[1].children[0].innerHTML += `<div class="playersContainer">
						<div class="topContainer">
							<div class="teamContainer">
								<div class="team saopaulo">
									<p class="teamInitials initialsBlack">${teste[j].team_initials}</p>
								</div>
							</div>
							<div class="infos">
								<div class="name" id='{players__player_id}'>${teste[j].players__player_name}</div>
								<div class="teams">
									<p class="firstTeam">${teste[j].matches__home_team}</p>
									<p class="versus">x</p>
									<p style='display:none'>${teste[j].teams__team_name}</p>
									<p class="secondTeam ">${teste[j].matches__visitor_team}</p>
								</div>
								<div class="date">
									<div class="dateValue">
										<svg width="10" height="12" viewBox="0 0 10 12" fill="none" xmlns="http://www.w3.org/2000/svg">
											<path d="M3.33333 5.1H2.22222V6.23333H3.33333V5.1ZM5.55556 5.1H4.44444V6.23333H5.55556V5.1ZM7.77778 5.1H6.66667V6.23333H7.77778V5.1ZM8.88889 1.13333H8.33333V0H7.22222V1.13333H2.77778V0H1.66667V1.13333H1.11111C0.494444 1.13333 0.00555555 1.64333 0.00555555 2.26667L0 10.2C0 10.8233 0.494444 11.3333 1.11111 11.3333H8.88889C9.5 11.3333 10 10.8233 10 10.2V2.26667C10 1.64333 9.5 1.13333 8.88889 1.13333ZM8.88889 10.2H1.11111V3.96667H8.88889V10.2Z" fill="#636363"/>
										</svg>
										<p>${teste[j].matches__match_date}</p>
									</div>
									<div class="hour">
										<svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
											<path d="M5.994 0.166672C2.774 0.166672 0.166504 2.78001 0.166504 6.00001C0.166504 9.22001 2.774 11.8333 5.994 11.8333C9.21984 11.8333 11.8332 9.22001 11.8332 6.00001C11.8332 2.78001 9.21984 0.166672 5.994 0.166672ZM5.99984 10.6667C3.4215 10.6667 1.33317 8.57834 1.33317 6.00001C1.33317 3.42167 3.4215 1.33334 5.99984 1.33334C8.57817 1.33334 10.6665 3.42167 10.6665 6.00001C10.6665 8.57834 8.57817 10.6667 5.99984 10.6667Z" fill="#636363"/>
											<path d="M6.2915 3.08333H5.4165V6.58333L8.479 8.42083L8.9165 7.70333L6.2915 6.14583V3.08333Z" fill="#636363"/>
										</svg>
										<p>${teste[j].matches__math_time}</p>
									</div>
								</div>
							</div>
							<div class="buttonContainer">
								<button class="button buyButton" id='${teste[j].players__player_id}'></button>
							</div>
						</div>
						<div class="lowerContainer">
							<div class="priceContainer">
								<div class="label">Preço</div>
								<div class="value" id='${teste[j].players__div_value}'>${teste[j].team_player__player_value}</div>
							</div>
							<div class="sections">
						 <div class="status">
									<div class="label">Status</div>
									<div class="value ${teste[j].players__situation}">
										</div>
								</div>
								<div class="divisor">
									<svg width="2" height="37" viewBox="0 0 2 37" fill="none" xmlns="http://www.w3.org/2000/svg">
										<line x1="0.833496" y1="0.5" x2="0.833494" y2="36.5" stroke="#DADADA" stroke-linecap="round"/>
									</svg>
								</div>
									   <div class="media">
								...
								<div class="matches">
									...
									<div class="value">${teste[j].match_player_status}</div>
								</div>
							</div>
						</div>
					</div>`
...
function disableBtnLineup() {
	...
	executeAction('get-limit-lineups', null, {identifier: user}).then((result) => {
		...
		for (let i = 0; i < result.length; i++) {
			if ((result[i].user_lineups === result[i].limit_lineups) && (window.location.href === ('http://fantasy.localhost:3004/fantasy-lineup/' + result[i].table_round_id))) {
